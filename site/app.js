import { bboxFromLandmarks, estimateGroundY, flightIntervals, computeFrameMetrics, runMonkeyCore } from './core.js?v=3.0.4';
...
function buildReport(){
 ...
 const gy=+$('groundSlider').value,fps=+$('sampleFps').value||30,flights=flightIntervals(state.frames,gy,fps),valid=state.frames.filter(f=>f.landmarks).length,l=state.frames.map(f=>f.metrics?.leftKnee?.value).filter(Number.isFinite),r=state.frames.map(f=>f.metrics?.rightKnee?.value).filter(Number.isFinite);
 state.report={schema:3,engine:state.version,created:new Date().toISOString(),targetSelection:state.runSelection,trackingStop:state.trackingStop,settings:{athlete:$('athlete').value,date:$('date').value,view:$('view').value,direction:$('direction').value,fps:+$('fps').value,sampleFps:fps,uncertaintyEnabled:$('uncertaintyEnabled').checked,pointSigmaPx:+$('sigmaPx').value,groundY:gy,clipSeconds:+$('clipSeconds').value},summary:{frames:state.frames.length,trackedFrames:valid,continuity:valid/state.frames.length,maxPeople:state.maxPeople,minLeftKnee:l.length?Math.min(...l):null,minRightKnee:r.length?Math.min(...r):null,flightIntervals:flights.length,manualCorrections:state.frames.reduce((n,f)=>n+(f.landmarks?.filter?.(p=>p?.manual).length||0),0)},flights,frames:state.frames.map(f=>({t:f.t,peopleCount:f.peopleCount,trackState:f.trackState,targetId:f.targetId??null,trackReason:f.trackReason??null,landmarks:f.landmarks,metrics:f.metrics}))};renderQuick();renderReport();
}
function renderQuick(){if(!state.report)return;const s=state.report.summary;$('quickStats').innerHTML=`${stat('指定選手',state.report.targetSelection?.id||'—')}${stat('追蹤連續率',(s.continuity*100).toFixed(1)+'%')}${stat('左膝最小角',s.minLeftKnee==null?'—':s.minLeftKnee.toFixed(1)+'°')}${stat('右膝最小角',s.minRightKnee==null?'—':s.minRightKnee.toFixed(1)+'°')}`;$('events').innerHTML=state.report.flights.length?state.report.flights.map((e,i)=>`<div class="event"><span>疑似雙腳離地 #${i+1} · ${formatTime(e.startTime)}–${formatTime(e.endTime)} · ${e.lowerMs.toFixed(0)}–${e.upperMs.toFixed(0)} ms</span><button data-seek="${e.startTime}">複查</button></div>`).join(''):'<p class="muted">未標記疑似雙腳離地；不代表已通過正式競走判定。</p>';document.querySelectorAll('[data-seek]').forEach(b=>b.onclick=()=>{if(!state.analyzing)video.currentTime=+b.dataset.seek;});}
